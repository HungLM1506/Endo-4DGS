...
def scene_reconstruction(mp, opt, hyper, pipe, testing_iterations, saving_iterations,
    ...
    if opt.dataloader:
        ...
        if opt.custom_sampler is not None:
            ...
            viewpoint_stack_loader = DataLoader(
                viewpoint_stack, batch_size=batch_size, sampler=sampler, num_workers=32, collate_fn=list)
            random_loader = False
        else:
            viewpoint_stack_loader = DataLoader(
                viewpoint_stack, batch_size=batch_size, shuffle=True, num_workers=32, collate_fn=list)
            random_loader = True
    ...
    for iteration in range(first_iter, final_iter+1):
        ...
        # Loss
        Ll1 = l1_loss(image_tensor, gt_image_tensor, None)
        ...
        if use_normal:
            ...
            pseudo_normal = get_pseudo_normal(gt_depth_tensor, None)
            pseudo_normal = F.interpolate(pseudo_normal, gs_normal.shape[2:4])
            normal_loss = mae_loss(
                gs_normal, pseudo_normal, None)*normal_weight
            loss += normal_loss
        ...
        if use_confidence:
            ...
            confidence_loss_img = confidence_loss(gt_image_tensor, image_tensor,
                                                  confidences, None)*un_img_weight
            confidence_loss_dep = confidence_loss(gt_depth_tensor/gt_depth_tensor.max(),
                                                  depth_tensor/depth_tensor.max(), confidences, None)*un_dep_weight
        ...
        if multi_scale:
            for i in range(len(gaussians.pool_list)):
                current_imgs = torch.stack(
                    [result['img'][i] for result in multi_scale_result], dim=0)
                current_deps = torch.stack(
                    [result['dep'][i] for result in multi_scale_result], dim=0)
                # current_imgs = current_imgs * mask_tensor
                # current_deps = current_deps * mask_tensor
                gt_depth_avg = avg_filter(gt_depth_tensor)
                gt_depth_avg = torch.nn.functional.interpolate(
                    gt_depth_avg, current_deps.shape[2:4])

                gd_loss = grad_loss(
                    current_deps, gt_depth_avg, mask=None)*grad_weight
                loss += gd_loss
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations):
                ...
                torch.save((gaussians.capture(), iteration), scene.model_path +
                           "/chkpnt" + f"_{stage}_" + str(iteration) + ".pth")
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc, renderArgs, stage, dataset_type, multi_scale):
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = ({'name': 'test', 'cameras': [scene.getTestCameras()[
                              idx % len(scene.getTestCameras())] for idx in range(10, 5000, 299)]},)  # ,
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, stage=stage,
                                        cam_type=dataset_type, *renderArgs, multi_scale=multi_scale)["render"], 0.0, 1.0)
                    if dataset_type == "PanopticSports":
                        gt_image = torch.clamp(
                            viewpoint["image"].to("cuda"), 0.0, 1.0)
                    else:
                        gt_image = torch.clamp(
                    ...
                    psnr_test += psnr(image.unsqueeze(0),
                                      gt_image.unsqueeze(0)).mean().double()
                ...
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {} SSIM {} lpips_score {}".format(
                    iteration, config['name'], l1_test, psnr_test, ssim_test, lpips_score_test))
                if tb_writer:
                    tb_writer.add_scalar(
                    ...
                    tb_writer.add_scalar(
                        stage+"/"+config['name'] + '/loss_viewpoint - ssim', ssim_test, iteration)
                    tb_writer.add_scalar(
                        stage+"/"+config['name'] + '/loss_viewpoint - lpips_score', lpips_score_test, iteration)

        if tb_writer:
            # tb_writer.add_histogram(f"{stage}/scene/opacity_histogram", scene.gaussians.get_opacity, iteration)
            tb_writer.add_scalar(f'{stage}/total_points',
                                 scene.gaussians.get_xyz.shape[0], iteration)
            tb_writer.add_scalar(f'{stage}/deformation_rate', scene.gaussians._deformation_table.sum(
            )/scene.gaussians.get_xyz.shape[0], iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+",
                        type=int, default=[500*i for i in range(100)])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[
                        1000, 3000, 4000, 5000, 6000, 7_000, 9000, 10000, 12000, 14000, 20000, 30_000, 45000, 60000])
