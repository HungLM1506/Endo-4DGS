...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")) and args.name is None:
            scene_info = sceneLoadTypeCallbacks["Colmap"](
                args.source_path, args.images, args.eval, args.use_pretrain)
        ...
        #     scene_info = sceneLoadTypeCallbacks["nerfies"](args.source_path, False, args.eval, args.use_pretrain)
        elif os.path.exists(os.path.join(args.source_path, "poses_bounds.npy")) and args.name == 'endonerf':
            scene_info = sceneLoadTypeCallbacks["endonerf"](
                args.source_path, args.white_background, args.eval, args.use_pretrain)
            ...
            scene_info = sceneLoadTypeCallbacks["scared"](
                args.source_path, args.white_background, args.eval, args.use_pretrain)
            print("Found point_cloud.obj, assuming SCARED data!")
        elif os.path.exists(os.path.join(args.source_path, "poses_bounds.npy")) and args.name == 'endo_custom':
        ...
        if self.loaded_iter:
            ...
            self.gaussians.create_from_pcd(
                scene_info.point_cloud, args.camera_extent, self.maxtime)

    def save(self, iteration, stage):
        if stage == "coarse":
            point_cloud_path = os.path.join(
                self.model_path, "point_cloud/coarse_iteration_{}".format(iteration))
        else:
            point_cloud_path = os.path.join(
