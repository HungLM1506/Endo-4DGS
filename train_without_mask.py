...
def scene_reconstruction(mp, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        if use_smooth:
            ...
            sm_loss = grad_loss(depth_tensor, gt_depth_tensor, mask=None) * grad_weight
            loss += sm_loss
        ...
        if use_normal:
            ...
            normal_loss = mae_loss(gs_normal, pseudo_normal, None)*normal_weight
            loss += normal_loss
        ...
        if multi_scale:
            for i in range(len(gaussians.pool_list)):
                ...
                gd_loss = grad_loss(current_deps, gt_depth_avg, mask=None)*grad_weight
                loss += gd_loss
